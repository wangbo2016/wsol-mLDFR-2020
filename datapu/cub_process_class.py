...
class CUBProcessor(torch.utils.data.Dataset):
    # Func:
    #   Constructor
    def __init__(self, dataset_root, dataset_type, lod_img_label=True, lod_obj_label=False):
        self.dataset_root_ = dataset_root
        self.dataset_type_ = dataset_type
        self.transformer_  = None
        # load category info
        self.obj_categories_ = []   # cidx2cname_
        self.obj_cid2cidx_   = OrderedDict()
    ...
    def load_category_info(self):
        ...
        for cinfo in obj_category_infos:
            ...
            cid   = int(tmp[0]) - 1
            cname = tmp[1]
            self.obj_cid2cidx_[cid] = len(self.obj_cid2cidx_)
            self.obj_categories_.append(cname)
        # return
    ...
    def load_split(self):
        ...
        # return
        if self.dataset_type_ == 'train': print('log: image num is %d' % (len(self.train_iid2iidx_)))
        else: print('log: image num is %d' % (len(self.test_iid2iidx_)))
    ...
    #   Format: list. We assume that the index is the image id, the list[index] is the image cid.
